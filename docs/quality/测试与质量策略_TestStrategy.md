...
`tests/unit/test_m3_generation.py` 已覆盖合法候选、缺参人工中断、未知字段/错误枚举、预算越权、非法 HOCON、有限修复、预算裁剪、Prompt 大小上限、瞬态重试、OpenAI-compatible Provider 和候选审查回答；`tests/unit/test_m5_quality.py` 覆盖质量拒绝、必填字段缺失和预算硬中断；`tests/unit/test_m5_engine.py` 覆盖提交、SeaTunnel 原生状态/指标转换、取消、清理、原子切换、回滚和错误脱敏；`tests/unit/test_m6_benchmark.py` 覆盖 L0 重复性、L1 故障注入、历史报告转换和 P0 拦截率；前端 `npm run build` 验证 Vue/TypeScript 产物。`tests/integration/test_m3_runtime.py` 提供 VM Checkpoint 和真实百炼 smoke test，默认通过配置开关跳过。当前 M6/M6.2 后端全量非集成测试为 63 项通过、2 项显式跳过；Ruff、Mypy、Alembic check 和 `uv lock --check` 均通过。SeaTunnel 2.3.10 已用 VM 合成 MySQL → Doris 影子表 → 原子 Swap/Rollback 验收；真实业务库、生产压测和百炼调用属于后续扩展。
